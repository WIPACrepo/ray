...
async def post_request(url: str, data: dict, headers: dict = None) -> dict:
    """
    Send an async POST request to a web server.
        ...
        async with asyncio.TaskGroup as tg:
            ...
            for word in words(f):
                ...
                if len(token_chunk) >= round(args.token_size * 0.9, 0) and "." in word:
                    ...
                    token_chunk = []
                    chunk_count = decrement(chunk_count)

                    payload = {"fr": args.fr, "to": args.to, "content": tokens}
                    results.append(tg.create_task(post_request(url=URL, data=payload)))
